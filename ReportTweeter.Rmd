...
followersDataFramePp <<- getFollowersDataFrame(tuserPp, 50)
```
...
# PP data frames with information of 50 friends
followersDataFramePsoe <<- getFollowersDataFrame(tuserPsoe, 50)
```
...
## Los Tweets
Es posible extender el análisis a los *Tweets* enviados por los perfiles de usuario de los partidos políticos. En el estudio efectuado, se han tomado los últimos 150 *Tweets* escritos por **PP** y **PSOE**.
```{r TweetsExtraction, echo=TRUE}

tweetsOfUserDataFramePsoe <<- getUserTweetsDataFrame(tuserPsoe, 150)
tweetsOfUserDataFramePp <<- getUserTweetsDataFrame(tuserPp, 150)
...
A nivel de la actividad global en red, se analizan los últimos 150 *Tweets* que tratan sobre el **PP** o el **PSOE** enviados dentro del territorio español.
...
if(dim(keyWordsDataFramePp)[1]>0){
  pie(c(sum(keyWordsDataFramePp[,2]), 
        10 - sum(keyWordsDataFramePp[,2])), 
      labels = c(paste ("Tweets con palabrotas - ",
                        sum(keyWordsDataFramePp[,2]/10)*100, "%")),
      radius = 1, col = c("yellow", "blue"), main="Tweets sobre PP")
}

if(dim(keyWordsDataFramePsoe)[1]>0){
  pie(c(sum(keyWordsDataFramePsoe[,2]), 
        10 - sum(keyWordsDataFramePsoe[,2])), 
      labels = c(paste ("Tweets con palabrotas - ",
                        sum(keyWordsDataFramePsoe[,2]/10)*100, "%")), 
